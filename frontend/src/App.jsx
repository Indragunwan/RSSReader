...
function App() {
  ...
  // Mobile and Frame states
  const [isSidebarOpen, setIsSidebarOpen] = useState(window.innerWidth > 1024);
  const [isArticleListOpen, setIsArticleListOpen] = useState(window.innerWidth > 768);
  ...
  return (
    <div className={`h-screen flex overflow-hidden font-sans ${isDarkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {/* Sidebar */}
      <aside 
        className={`flex-shrink-0 border-r border-slate-200 dark:border-slate-800 flex flex-col bg-white dark:bg-slate-900 transition-all duration-300 ease-in-out z-[70] 
      ...
      >
        <div className="p-4 flex items-center justify-between min-w-[256px]">
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={() => setIsSidebarOpen(false)} className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
              ...
              <div key={category.id} className="space-y-1">
                <div 
                  onClick={() => {
                    ...
                    // On mobile, if folder is clicked, we might want to stay in sidebar if it's just expanding, 
                    // or go to list if we consider it a selection. Let's stay for now.
                  }}
                      ...
                      <div key={feed.id} className="group/feed relative">
                        <button 
                          onClick={(e) => { 
                            ...
                            FeedService.getArticles({ limit: 50, feed_id: feed.id }).then(res => setArticles(res.data)); 
                            if (isMobile) {
                              setIsSidebarOpen(false);
                              setIsArticleListOpen(true);
                            }
      ...
      {/* Article List */}
      <main 
        className={`flex-shrink-0 border-r border-slate-200 dark:border-slate-800 flex flex-col bg-white dark:bg-slate-900 transition-all duration-300 ease-in-out
          ${isMobile && !isArticleListOpen && selectedArticle ? 'hidden' : 'flex'}
          ${isMobile ? 'w-full' : (isArticleListOpen ? 'w-96' : 'w-0 opacity-0 overflow-hidden border-none')}`}
      >
        <header className="p-4 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md">
          <div className="flex justify-between items-center mb-1">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <h2 className="font-bold text-lg dark:text-slate-100 truncate">
      ...
      {/* Reader View */}
      <section className={`flex-1 bg-white dark:bg-slate-950 overflow-y-auto ${isMobile && isArticleListOpen ? 'hidden' : 'block'}`}>
        {selectedArticle ? (
          <motion.div 
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="max-w-3xl mx-auto px-4 sm:px-8 py-6 sm:py-12"
          >
            <div className="mb-8 border-b border-slate-100 dark:border-slate-800 pb-8">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-sm">
                   {isMobile && (
                     <button onClick={() => setIsArticleListOpen(true)} className="p-2 -ml-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500">
                       <ArrowLeft size={20} />
                     </button>
                   )}
                   {!isMobile && !isSidebarOpen && (
                ...
                <div className="flex items-center gap-1">
                  {!isMobile && (
                    <button 
                      onClick={() => setIsArticleListOpen(!isArticleListOpen)} 
                      className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400"
                      title={isArticleListOpen ? "Hide list" : "Show list"}
                    >
                      {isArticleListOpen ? <Minimize2 size={18} /> : <Maximize2 size={18} />}
                    </button>
                  )}
                </div>
              </div>
              <h1 className="text-2xl sm:text-4xl font-extrabold text-slate-900 dark:text-slate-100 leading-tight mb-6">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-4">
                <a 
                  ...
                  className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-3 sm:px-4 py-2 rounded-lg text-sm font-semibold transition-colors shadow-lg shadow-orange-500/20"
                >
                  Visit <ExternalLink size={14} className="hidden sm:inline" />
                </a>
                <button 
                  onClick={(e) => toggleFavorite(e, selectedArticle)}
                  className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 px-3 sm:px-4 py-2 rounded-lg text-sm font-semibold transition-colors"
                >
                  <Star size={14} fill={selectedArticle.favorite ? "currentColor" : "none"} />
                  <span className="hidden sm:inline">{selectedArticle.favorite ? 'Saved' : 'Save for Later'}</span>
                </button>
                <button 
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 p-4">
             <button 
                onClick={() => setIsSidebarOpen(true)}
                className="absolute top-4 left-4 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 lg:hidden"
              >
                <Menu size={24} />
              </button>
             <Rss size={64} className="mb-4 opacity-10" />
             <p className="text-lg text-center">Select an article to start reading</p>
          </div>
        )}
      </section>
            ...
            <div className="p-6">
                 ...
                 <div className="relative mb-6 flex gap-2">
                   ...
                   <button 
                    ...
                    className="bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white px-4 py-2 rounded-xl font-bold transition-colors flex-shrink-0"
                   >
                     Search
                   </button>
                 </div>
                 ...
                 {loading ? (
                   ...
                   <div className="py-8 text-center text-slate-400 dark:text-slate-600 text-sm">
                     ...
                     <p>Paste a URL and search for feeds</p>
                   </div>
                 )}
            </div>
          </div>
